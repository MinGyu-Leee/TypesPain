...
const SUFFERING_KW: KWEntry<SufferingKey>[] = [
  ...
  { key: '구부득고', terms: [
    ...
    // 원하는 것을 사줌/해줌/받지 못함 (타인 미충족)
    ['안사줘', 2], ['안 사줘', 2], ['못 사줘', 2],
    ['안사줬', 2], ['안 사줬', 2], ['못 사줬', 2],
    ['못 받았', 2], ['안 받았', 2],
    ['안 해줬', 2], ['못 해줬', 2],
    ['안 지켰', 2], ['못 지켰', 2],   // 약속을 안 지켰어요
    ['안 들어줬', 2], ['못 들어줬', 2], // 부탁을 안 들어줬어요
  ]},
  { key: '오온성고', terms: [
...
const SUFFERING_PATTERNS: PatternEntry[] = [
  ...
  // ─ 원증회고: [관계인물]랑/이랑 안 맞음 (사이가 없이 바로 "안 맞아요") ─
  {
    key: '원증회고',
    re: /(엄마|아빠|부모님|남편|아내|가족|형|언니|오빠|동생|친구|상사|룸메이트)[\s\S]{0,8}(안\s*맞|잘\s*안\s*맞|안맞)/,
    score: 2,
  },
  // ─ 원증회고: [가족] 싫음/밉/갈등 ─
  {
  ...
  // ─ 구부득고: 목적어(을/를) + 안/못 + 사/해/받 동사 (물건·서비스 미충족) ─
  { key: '구부득고', re: /(을|를)[\s\S]{0,10}(안|못)\s*(사줘|사줬|해줘|해줬|받았|줬|줘)/, score: 3 },

  // ─ 구부득고: 약속을 안 지킴 ─
  { key: '구부득고', re: /(약속)[\s\S]{0,10}(안|못)\s*(지켰|지켜|지킴|지키)/, score: 3 },
